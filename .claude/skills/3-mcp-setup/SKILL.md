...
토큰을 받으면 `claude mcp add` CLI로 MCP 설정에 추가:
```bash
claude mcp add -e SLACK_BOT_TOKEN={사용자가 입력한 토큰} slack -- npx @modelcontextprotocol/server-slack
```
...
각 Step에서 `claude mcp add` CLI로 개별 등록했으므로, 별도의 통합 저장은 필요 없습니다.

`claude mcp list` 명령으로 최종 상태를 확인한 뒤, 사용자에게 결과를 표로 보여주세요:
