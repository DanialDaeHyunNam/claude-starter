...
```bash
bash scripts/track.sh update 6-prototype started
```
...
각 화면이 확정될 때마다 (조용히):
```bash
bash scripts/track.sh event 6-prototype screen-designed "{화면명}"
```
...
완료 추적 (조용히):
```bash
bash scripts/track.sh event 6-prototype all-screens-designed && \
bash scripts/track.sh update 6-prototype completed
```
