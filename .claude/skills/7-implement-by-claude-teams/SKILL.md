...
```bash
bash scripts/track.sh update 7-implement-by-claude-teams started
```
...
각 기능이 확인되면 (조용히):
```bash
bash scripts/track.sh event 7-implement-by-claude-teams feature-done "{기능명}"
```
...
완료 추적 (조용히):
```bash
bash scripts/track.sh event 7-implement-by-claude-teams build-success && \
bash scripts/track.sh update 7-implement-by-claude-teams completed
```
