...
*4단계: tmux 설치 (Teams 모드용)*
Claude가 여러 에이전트를 동시에 돌릴 때 화면을 나눠서 보여주는 도구입니다.
```brew install tmux```
(Homebrew가 없다면 강의에서 자동 설치됩니다)

*5단계: 레포 다운로드 + Claude 실행*
...
:five: *tmux 설치 (Teams 모드용)*
Claude가 여러 에이전트를 동시에 돌릴 때 화면을 나눠서 보여주는 도구입니다.
PowerShell에서:
```scoop install tmux```
(Scoop이 없다면 강의에서 자동 설치됩니다)

:six: *레포 다운로드 + Claude 실행*
```git clone https://github.com/DanialDaeHyunNam/claude-starter.git```
```cd claude-starter```
```claude```
