...
```bash
bash scripts/track.sh update 9-deploy started
```
...
배포 완료 후 (조용히):
```bash
bash scripts/track.sh event 9-deploy deployed "{Vercel URL}"
```
...
완료 추적 (조용히):
```bash
bash scripts/track.sh event 9-deploy auto-deploy-configured && \
bash scripts/track.sh update 9-deploy completed
```
